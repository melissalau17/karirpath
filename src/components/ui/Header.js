...
const Header = () => {
    ...
    useEffect(() => {
		const checkSession = async () => {
			const { data, error } = await supabase.auth.getSession(); // Dapatkan sesi
			if (data.session) {
				setIsLoggedIn(true); 
			} else {
				setIsLoggedIn(false); 
        ...
        <header style={styles.header}>
            <div style={styles.logoNavContainer}>
                ...
                <nav style={styles.nav}>
                    ...
                    {isLoggedIn && <Link to="/my-jobs" style={styles.navLink}>Your Jobs</Link>}
                </nav>
            </div>
            <div style={styles.authButtons}>
                {isLoggedIn ? (
                    <>
                        <button onClick={handleLogoutClick} style={styles.signInButton}>Logout</button>
                    </>
                ) : (
